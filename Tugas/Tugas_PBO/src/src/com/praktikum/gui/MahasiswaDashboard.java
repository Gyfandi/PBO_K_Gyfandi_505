...
public class MahasiswaDashboard {
    ...
    private void initializeUI() {
        ...
        HBox reportFields = new HBox(10, new Label("Nama Barang:"), itemNameField, new Label("Deskripsi:"), itemDescriptionField, new Label("Lokasi:"), itemLocationField, reportButton);
    ...
    private void updateReportedItemsList() {
        ...
        for (Item item : DataStore.getReportedItems()) { // Gunakan DataStore untuk mendapatkan list
            ...
            if (item.getreported().equals(currentUser) && "Reported".equals(item.getStatus())) { //
                items.add("Nama: " + item.getitemName() + " | Lokasi: " + item.getlocation() + " | Deskripsi: " + item.getdescription()); //
            }
        }
        if (items.isEmpty()) {
